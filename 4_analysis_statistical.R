...
#-------------------------------------------------------------------------------
#Plot Density
data %>% ggplot(aes(x = DTC_rate, y = Own_type, group = Own_type)) + 
...
summary(aov(data$DTC_rate ~ data$Own_type))
...
# Cost vs Star rating
scat + geom_point(aes(y=Admissions), color = 'navy', alpha=0.3) + 
  geom_density2d(aes(y=Admissions),color='orange')
